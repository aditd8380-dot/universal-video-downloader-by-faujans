const API = "https://universal-video-downloader-by-faujans-production.up.railway.app";
...
analyzeBtn.onclick = async () => {
  ...
  try {
    ...
    $("duration").textContent =
      data.duration
        ? `Duration: ${Math.round(data.duration)} detik`
        : "";
...
async function pollJob(jobId) {
  const timer = setInterval(async () => {
    ...
    try {
      ...
      showMessage(
        "Tidak dapat membaca status download.",
        true
      );
...
cancelBtn.onclick = async () => {
  ...
  try {
    ...
    await fetch(
      `${API}/api/download/${currentJob}/cancel`,
      {
        method: "POST"
      }
    );
